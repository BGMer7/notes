...
1. 直接将图片以二进制的形式存储在数据库中。例如MySQL提供将文件类型设置为blob二进制类型，然后将图片存放在数据库中。

   如果通过文本形式将图片的Base64编码存储在数据库中，占用的空间会比图片本身的大小还大。

2. 将图片存储在硬盘或者文件服务器上，数据库将存储的路径保存下来。

基本绝大多数的解决方案都采用第二种方式。

首先，视频、声音、图片为非结构化数据，传统的关系型数据库主要是是针对结构化数据设计的，虽然有相应的字段格式支持存储，即：如果要把视频等文件存储到数据库的话，要使用大字段(例如：Oracle，Mysql中的[BLOB](http://baike.baidu.com/link?url=OmQFzKxhsXa3vZjn_NFhszZzOYL-adZaSYYGFF8-LSaGZPFwT1Swg0IeZ6Wxd5WVHw50Q7pRjkpJMAmFJieyc_)特殊二进制字段)，但性能表现不佳。当然也可以采用非结构化的NoSQL数据库，但是现有的NoSQL数据的存储单元仍是针对小块存储设计的，也就是说，当面对较大的视频格式等文件时，还是会有性能问题。
